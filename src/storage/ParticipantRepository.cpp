...
ParticipantRepository::ParticipantRepository(IDatabaseManager *dataManager, QObject *parent)
...
bool ParticipantRepository::addParticipant(const Participant &p) {
    ...
    QString checkQuery = QString("SELECT id, created_at FROM %1 LIMIT 1").arg(TableNamesSQLite::PARTICIPANTS);
